...
1. **[ISSUE-168] Root Invariant Erosion in Chat RLHF (`test/geomind/chat.cl`)**:
   ...
   - Fix: Whitelist Domain 0 edges (`e_idx >= 4.0`), decaying only conversational/dynamic domain edges with `min_w = 0.10`.
...
3. **[ISSUE-170] Salient Attractor Bank Leak in Hopfield Relaxation (`src/std/resonator.cl`)**:
   ...
   - Fix: Implement `cartan_tree_free` / `collections_free_tree` and free `chosen_bank` before return.
...
6. **[ISSUE-173] `cargraph_sleep_consolidate_file` Drops Base Topology & Leaks Builders (`src/std/cargraph_consolidate.cl`)**:
   ...
   - Fix: Extract base CSR topology directly from `cg`; serialize consolidated CSR edges into binary file; free all intermediate builders and graphs via `csr_graph_free`, `cargraph_builder_free`, `cargraph_free`.
